...
- V4 分层配置默认值、V2/V3 显式迁移、显示方式校验、严格版本拒绝、命名自定义引擎校验、Fun-ASR 热词约束和未知扩展字段保留。
- 从 V4 `EngineConfig` 为 Gummy、Vosk、SOSV、GLM、Fun-ASR 及自定义引擎生成启动参数，并验证关闭翻译后不传翻译服务参数。
...
- Electron `userData/config.json` 的真实磁盘读写、V2→V3→V4/V3→V4 迁移和无版本配置回退的桌面端流程。
