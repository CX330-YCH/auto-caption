...
选择该引擎或切换源语言时，软件会立即显示该语言的资源状态。源语言使用本地化名称，例如“简体中文（中国大陆）”“繁体中文（台湾）”；内部 locale 统一为 BCP-47 连字符格式，系统返回的 `zh_CN` 与配置中的 `zh-CN` 会视为同一语言。只有“已就绪”才允许启动字幕引擎。系统未报告语言资源时显示“未下载”；系统已有资源但当前模块尚未就绪时显示“待启用”。两者都通过独立弹窗明确执行“准备/启用语言资源”，macOS 可能复用已有资源，也可能下载缺失内容，准备进度不计入默认 30 秒字幕引擎启动超时。资源名额已满时，弹窗列出已保留语言供用户明确释放。资源由 macOS 存储、共享和更新，状态不写入 Auto Caption 配置文件；源语言列表以当前系统查询结果为准，不支持自动检测。
...
当前版本使用带 `schemaVersion: 7` 的分层配置文件。V2–V6 配置会依次迁移到 V7；V6→V7 将翻译开关、目标语言和 Provider 专属配置移到独立的 `engine.translation` 层，不改变原有 Google/Ollama 选择。无版本配置仍使用默认设置。

字幕引擎设置会根据当前识别引擎显示其源语言和专属字段。关闭“启用翻译”时不显示或传递翻译参数；开启后可在独立的“配置翻译引擎”中选择 Google 或 Ollama，并编辑所选 Provider 的字段。识别与翻译配置分别保存，切换识别引擎不会覆盖翻译设置。Microsoft Azure Translator 目前只有配置元数据，选项处于禁用状态且不会发起请求。“更多设置”显示当前识别引擎需要的凭据或本地模型路径，以及通用录音路径和启动超时。修改仍需点击“应用更改”才会保存。
...
“日志记录”页仍只显示 INFO、WARN 和 ERROR。通用设置中的 Debug Mode 开启后立即记录 Electron、Renderer、Python、SDK 和 Apple Speech helper 的脱敏完整错误、协议事件、每帧音频元数据、队列深度/延迟、Provider 输入状态、翻译积压和进程资源指标；正在运行的内置引擎无需重启。日志可能包含识别文本、翻译文本和本地路径，但 API Key、Token、密码、Cookie、Authorization 及 PCM 音频正文始终不会写入。日志页显示开关状态、写入健康和文件大小；“保存完整 Debug 日志”导出点击时为止的 `.jsonl`，“清空”只清表格。
