...
The current version uses a layered configuration file with `schemaVersion: 7`. V2–V6 configurations migrate explicitly to V7; V6→V7 moves the translation switch, target language, and provider-specific settings into an independent `engine.translation` layer without changing the existing Google/Ollama selection. Unversioned configuration still falls back to defaults.

Caption engine settings show the source languages and provider fields for the selected recognition engine. Disabling translation hides and omits translation parameters. After enabling it, the independent "Configure Translation Engine" panel selects Google or Ollama and shows that provider's fields. Recognition and translation configurations are stored separately, so changing recognition engines does not overwrite translation settings. Microsoft Azure Translator currently has metadata only; its option is disabled and no request is sent. "More Settings" shows the recognition engine's credentials or local model path plus shared settings. Changes are saved only after clicking "Apply Changes."
