...
現在のバージョンは `schemaVersion: 3` の階層化設定を使用します。完全な V2 設定は自動移行され、旧カスタムエンジンも名前付き項目に変換されます。バージョンなし設定は引き続き既定値になります。
...
下の図は字幕表示ウィンドウです。同じ文のストリーミング字幕はその場で増分更新され、確定後の文が遅延した中間結果で上書きされることはありません。折り返しを有効にすると、現在のウィンドウ幅、フォント、文字サイズによる実際の組版に従って正確に分行します。右側のツールバーは自動的に隠れ、ポインターがウィンドウまたはツールバーに入るか、キーボードフォーカスを受けると再表示されます。3つのボタンは、字幕表示ウィンドウを閉じる、字幕制御ウィンドウを開く、マウス透過を有効化する機能です。ウィンドウの左右の端をドラッグして幅を調整できます。
