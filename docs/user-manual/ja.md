...
選択すると、ソース言語のシステムモデル状態を直ちに確認します。`インストール済み` の場合だけ字幕エンジンを開始できます。ダウンロード可能な場合は専用モデルダイアログから開始し、macOS の進捗が完了するまで待ってください。モデル取得は既定の30秒起動タイムアウトに含まれません。予約上限に達した場合は、予約済み言語を明示的に解放してからダウンロードできます。モデルは macOS が保存、共有、自動更新し、Auto Caption の設定には状態を保存しません。ソース言語は実行時の照会結果から表示され、自動検出はありません。
