...
## macOS システム音声認識を使用する前の準備

このローカルエンジンは Apple の `SpeechAnalyzer`、`SpeechTranscriber`、`AssetInventory` を使用し、macOS 26 以降が必要です。macOS 以外では表示されません。macOS で OS バージョン、ネイティブヘルパー、ハードウェア機能、または実行時の対応言語が条件を満たさない場合、項目は灰色のまま表示され、クリックしても理由を示すだけで選択は変わりません。

このエンジンの選択時またはソース言語の変更時に、言語リソース状態を直ちに表示します。メニューは「簡体字中国語（中国本土）」「繁体字中国語（台湾）」などのローカライズ名を使用します。内部 locale は BCP-47 のハイフン形式へ統一され、システムの `zh_CN` と設定の `zh-CN` は同じ言語として扱われます。`準備完了` の場合だけ字幕エンジンを開始できます。macOS がインストール済みとして報告しない場合は「未ダウンロード」、システムに資源があっても現在のモジュールが未準備なら「有効化待ち」と表示します。どちらも専用ダイアログから明示的に準備または有効化します。macOS は既存資源を再利用するか、不足分をダウンロードし、準備進捗は既定の30秒起動タイムアウトに含まれません。予約上限時は予約済み言語を明示的に解放できます。資源は macOS が保存、共有、更新し、状態は Auto Caption 設定に保存しません。自動言語検出はありません。

認識はデバイス上で行われ、外部翻訳は final 字幕ごとに一度だけ実行されます。システム出力の取得には引き続き後述の BlackHole 設定が必要で、ScreenCaptureKit 取得は追加しません。
...
字幕コントロールウィンドウの最小サイズは 900×600 です。コンテンツ領域の幅が 1200px 未満になると、設定パネルは左側の「設定」レールに折りたたまれます。マウスを合わせるかキーボードでフォーカスすると一時的に開き、クリックすると開いた状態を固定できます。もう一度クリックする、設定領域の外側をクリックする、または Esc を押すと閉じます。未適用の字幕エンジンと字幕スタイルの下書きは、パネルの開閉やレスポンシブ境界をまたぐサイズ変更でも失われません。設定フォームの最大幅は 640px で、左側領域を広げてもラジオボタン、セレクト、入力欄、スライダーが際限なく引き伸ばされることはありません。通常の項目はフォーム幅が 360px を超える場合にラベルとコントロールを同じ行に保ち、360px 以下の場合のみ共通の上下配置に切り替わります。言語・テーマ・色・字幕行数はまとまった単位として配置されます。広い領域ではスイッチも通常項目と同じ右揃えのラベル列を使用します。ホバー展開など 360px 以下の狭い領域では、コンテンツ左端から「ラベル + スイッチ」を左揃えのコンパクトな一行として表示するため、ペア全体が中央へずれたり、ラベルとスイッチが離れたりしません。字幕スタイルの原文・翻訳フォントでは、現在のシステムがアプリに列挙を許可したローカルフォントを検索できます。フォント名は一覧を初めて開いた時だけ読み取り、ローカルメモリ内だけで使用します。フォントファイルやフォント一覧をアップロードすることはありません。列挙が未対応または拒否された場合や、設定したフォントがインストールされていない場合は、手動入力に切り替えて単一のフォント名または完全な CSS フォントスタックを引き続き利用できます。列挙失敗で既存値が上書きされることはありません。フォント変更もスタイルの下書きであり、「適用」を押した後に字幕ウィンドウへ反映されます。字幕スタイルのプレビューは右ペイン下部に固定され、高さはコンテンツ領域の 35% までです。超過した内容はプレビュー領域内だけでスクロールし、字幕記録を覆いません。

現在のバージョンは `schemaVersion: 7` の階層化設定を使用します。V2～V6 設定は明示的に V7 へ移行され、V6→V7 は翻訳スイッチ、対象言語、Provider 固有設定を独立した `engine.translation` 層へ移します。既存の Google/Ollama 選択は変わりません。バージョンなし設定は引き続き既定値になります。

字幕エンジン設定は、選択した認識エンジンのソース言語と専用項目を表示します。翻訳を無効にすると翻訳項目と起動引数は非表示・未送信になります。有効にすると、独立した「翻訳エンジンを設定」で Google または Ollama を選択し、その Provider の項目を編集できます。認識と翻訳の設定は別々に保存されるため、認識エンジンを切り替えても翻訳設定は上書きされません。Microsoft Azure Translator は現在メタデータのみで、選択肢は無効、リクエストも送信されません。「詳細設定」には認識エンジンの資格情報またはローカルモデルパスと共通設定が表示されます。変更は「変更を適用」で保存されます。
...
下の図は字幕表示ウィンドウです。同じ文のストリーミング字幕はその場で増分更新され、確定後の文が遅延した中間結果で上書きされることはありません。字幕スタイルでは「文単位表示」と「行単位スクロール」を選択できます。文単位表示は従来の最新字幕を表示する方式を保持し、両方の表示方式で文字を左右対称の安全余白内に表示します。行単位スクロールは現在の内容幅、フォント、文字サイズで常に正確に折り返します。幅不足による自動折り返しで完成した行だけを両端揃えにし、文境界で終わる行と現在の最終行は左揃えを保ちます。原文と翻訳文はそれぞれ「字幕行数」で指定した固定の表示行容量を確保して独立にスクロールします。文字が少ない場合は字幕を複製せず空き行を残し、遅延した翻訳文が原文を押し出すこともありません。ストリーミング partial が短縮または書き換えられても、すでに画面外へ流れた古い行が表示行数を埋めるために戻ることはありません。書き換え後の現在テキストが以前の表示位置を一時的に越えない場合は空き行を保ち、その後の増分または次の字幕で再び埋まります。行単位スクロールでは「文ごとに改行」と「連続レイアウト」も選択できます。前者は各字幕を新しい行から表示し、後者は実際の幅で折り返すまで次の字幕を前の文末に続けます。1つの選択が原文と翻訳文に同時に適用されますが、両トラックは独立したままです。末尾の増分または新しい字幕によって新しい行が現れた場合だけ、同じトラックの古い行を約 500ms で上へ移動します。partial の書き換え、確定状態への更新、過去の翻訳の追加は表示位置を動かさず、ウィンドウ幅やフォントの変更は新しいレイアウトで再配置します。OS の「視差効果を減らす」が有効な場合はアニメーションしません。右側のツールバーは自動的に隠れ、ポインターがウィンドウまたはツールバーに入るか、キーボードフォーカスを受けると再表示されます。ツールバーはオーバーレイであり、字幕のレイアウト幅を消費しません。3つのボタンは、字幕表示ウィンドウを閉じる、字幕制御ウィンドウを開く、マウス透過を有効化する機能です。字幕ウィンドウの高さは表示方式、行容量、フォントから自動計算して固定されるため、手動で縦方向に縮めて文字を切ることはできません。左右の端は引き続きドラッグでき、幅変更後の再折り返しに合わせて高さも自動更新されます。
...
「ログ記録」画面は INFO、WARN、ERROR を表示します。一般設定の Debug Mode を有効にすると、Electron、Renderer、Python、SDK、Apple Speech helper のマスキング済み完全エラー、プロトコルイベント、フレーム単位の音声メタデータ、キュー深度/遅延、Provider 入力状態、翻訳滞留、プロセスメトリクスを直ちに記録します。実行中の内蔵エンジンは再起動不要です。認識文、翻訳文、ローカルパスを含む場合がありますが、API Key、Token、Password、Cookie、Authorization、PCM 音声本体は記録しません。ログ画面にはモード、書き込み状態、サイズを表示し、保存はその時点までの `.jsonl` を出力します。クリアは表だけを消去します。
