...
現在のバージョンは `schemaVersion: 2` を持つ階層化設定ファイルを使用し、以前のバージョンなし形式は読み込みません。旧設定で初回起動した場合は既定値を使用し、アプリ終了時に新しい V2 設定を書き込みます。旧設定は自動移行されません。

字幕エンジン設定には、選択中のエンジンが対応する言語、翻訳オプション、Provider 固有フィールドだけが表示されます。「詳細設定」には選択中のエンジンの API キーまたはローカルモデルパスと、共通の録音パス・起動タイムアウトが表示されます。エンジンを切り替えても、他のエンジンで保存済みの固有設定は削除されません。変更の保存には引き続き「変更を適用」をクリックする必要があります。
...
ソフトウェアには5つの内蔵字幕エンジン（`gummy`、`glm`、`vosk`、`sosv`、`fun_asr`）が用意されています。他の字幕エンジンが必要な場合、カスタムエンジンオプションを有効にすることで呼び出すことができます（他のエンジンはこのソフトウェア向けに特別に開発する必要があります）。エンジンパスはコンピュータ上のカスタム字幕エンジンの場所を指し、エンジンコマンドはカスタム字幕エンジンの実行パラメータを表します。これらは該当する字幕エンジンの規則に従って設定する必要があります。
