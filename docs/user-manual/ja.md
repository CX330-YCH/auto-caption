...
現在のバージョンは `schemaVersion: 4` の階層化設定を使用します。完全な V2 設定は最初に名前付きカスタムエンジンへ移行され、その後 V2/V3 とも V4 へ移行します。新しい表示方式の既定値は「文単位表示」のため、アップグレードしても従来の字幕動作は変わりません。バージョンなし設定は引き続き既定値になります。
...
下の図は字幕表示ウィンドウです。同じ文のストリーミング字幕はその場で増分更新され、確定後の文が遅延した中間結果で上書きされることはありません。字幕スタイルでは「文単位表示」と「行単位スクロール」を選択できます。文単位表示は従来の最新字幕を表示する方式を保持します。行単位スクロールは現在のウィンドウ幅、フォント、文字サイズで常に正確に折り返し、文字を左端から表示します。原文と翻訳文はそれぞれ「字幕行数」で指定した表示行数を保持して独立にスクロールするため、遅延した翻訳文が原文を押し出すことはありません。新しい行が現れると、同じトラックの古い行を約 500ms のアニメーションで上へ移動します。OS の「視差効果を減らす」が有効な場合はアニメーションしません。右側のツールバーは自動的に隠れ、ポインターがウィンドウまたはツールバーに入るか、キーボードフォーカスを受けると再表示されます。3つのボタンは、字幕表示ウィンドウを閉じる、字幕制御ウィンドウを開く、マウス透過を有効化する機能です。ウィンドウの左右の端をドラッグして幅を調整できます。
