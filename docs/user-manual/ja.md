...
下の図は字幕表示ウィンドウです。同じ文のストリーミング字幕はその場で増分更新され、確定後の文が遅延した中間結果で上書きされることはありません。字幕スタイルでは「文単位表示」と「行単位スクロール」を選択できます。文単位表示は従来の最新字幕を表示する方式を保持します。行単位スクロールは現在のウィンドウ幅、フォント、文字サイズで常に正確に折り返し、文字を左端から表示します。原文と翻訳文はそれぞれ「字幕行数」で指定した表示行数を保持して独立にスクロールするため、遅延した翻訳文が原文を押し出すことはありません。行単位スクロールでは「文ごとに改行」と「連続レイアウト」も選択できます。前者は各字幕を新しい行から表示し、後者は実際の幅で折り返すまで次の字幕を前の文末に続けます。1つの選択が原文と翻訳文に同時に適用されますが、両トラックは独立したままです。末尾に新しい行が現れると同じトラックの古い行を約 500ms で上へ移動し、過去の翻訳の追加、ウィンドウ幅、フォントの変更は新着字幕として扱わず再レイアウトします。OS の「視差効果を減らす」が有効な場合はアニメーションしません。右側のツールバーは自動的に隠れ、ポインターがウィンドウまたはツールバーに入るか、キーボードフォーカスを受けると再表示されます。3つのボタンは、字幕表示ウィンドウを閉じる、字幕制御ウィンドウを開く、マウス透過を有効化する機能です。ウィンドウの左右の端をドラッグして幅を調整できます。
