...
字幕コントロールウィンドウの最小サイズは 900×600 です。コンテンツ領域の幅が 1200px 未満になると、設定パネルは左側の「設定」レールに折りたたまれます。マウスを合わせるかキーボードでフォーカスすると一時的に開き、クリックすると開いた状態を固定できます。もう一度クリックする、設定領域の外側をクリックする、または Esc を押すと閉じます。未適用の字幕エンジンと字幕スタイルの下書きは、パネルの開閉やレスポンシブ境界をまたぐサイズ変更でも失われません。設定パネルが 480px 以下の場合、フォームは統一された上下配置になり、言語・テーマ・色・字幕行数・スイッチは途中で分断されず、まとまった単位として配置されます。字幕スタイルのプレビューは右ペイン下部に固定され、高さはコンテンツ領域の 35% までです。超過した内容はプレビュー領域内だけでスクロールし、字幕記録を覆いません。
