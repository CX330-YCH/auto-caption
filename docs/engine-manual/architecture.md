...
`CaptionViewport` 继续作为整句显示和逐行滚动的唯一分派入口。滚动模式不再为每个 `CaptionItem` 复制一套隐藏测量节点，而是先由纯 TypeScript `captionTracks.ts` 构建原文、译文两个 segment 轨道，再分别交给同一个 `RollingCaptionTrack.vue`：

```text
CaptionItem[]
├── source segments      ─┐
└── translation segments ─┴─> compose boundary mode
                              -> Chromium visual slices
                              -> stable captionId/offset rows
                              -> independent last-N animation
```

`sentence` 和 `continuous` 只改变 segment 之间使用硬换行还是语言感知连接符；partial/final、异步翻译顺序、精确换行和动画选择均走同一管线。视觉行 key 使用行首 `captionId + captionOffset`，避免把同一 partial 行的文字修订当成新行。轨道按实测视觉行向前扩展并在稳定行首建立裁剪锚点，同时限制 segment 和字符窗口，禁止字幕会话增长导致隐藏 DOM 无界。
