...
export interface CaptionItem {
  command: "caption",
  event_version: 1, // ライフサイクルフィールドのバージョン
  phase: "partial" | "final",
...
同じ文の中間結果と最終結果は同じ `index` を再利用する必要があります。`partial` は更新可能、`final` は確定済みを表します。新しいエンジンは `event_version: 1` と `phase` を必ず同時に指定してください。旧カスタムエンジンとの互換性のため両方をまとめて省略できますが、片方だけの指定は無効です。完全な規則はプロセスプロトコルを参照してください。
...
python main.py -e fun_asr -s ja -t zh -a 0 -c 10 \
  ...
  -fvmodel fun-asr-realtime -fcontext "Auto Caption" -fcontext "阿里雲百煉"
```
