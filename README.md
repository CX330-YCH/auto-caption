<div align="center" >
    ...
    <p><i>v2.14.0 版本已经发布，包含 macOS arm64 构建...</i></p>
</div>
...
- 支持保留原有整句显示，或切换为基于精确换行的逐行滚动字幕
- 灵活的字幕引擎选择（阿里云 Gummy 云端模型、GLM-ASR 云端模型、本地 Vosk 模型、本地 SOSV 模型、还可以自己开发模型）
- 多语言识别与翻译（见下文“⚙️ 自带字幕引擎说明”）
- 字幕记录展示与导出（支持导出 `.srt` 和 `.json` 格式）
- INFO/WARN/ERROR 日志页与隐式完整 Debug 会话分离，所有字幕引擎、SDK 回调、异常栈和 stderr 均可按本次启动导出为脱敏 `.jsonl` 诊断日志

## 📖 基本使用

> ⚠️ 注意：当前 v2.14.0 已提供 Windows 和 macOS arm64 构建；Linux 仍需从源码或现有构建链路验证。
