<div align="center" >
    ...
    <p><i>v2.14.0 がリリースされました。macOS arm64 ビルドを含みます...</i></p>
</div>
...
- 従来の文単位表示を保持するか、正確な折り返しに基づく行単位スクロール字幕へ切り替え可能
...
- INFO/WARN/ERROR のログ画面と非表示の完全 Debug セッションを分離し、すべての字幕エンジン、SDK コールバック、例外 traceback、stderr を起動単位のサニタイズ済み `.jsonl` 診断ログとして保存可能

## 📖 基本使い方

> ⚠️ 注意：現在の v2.14.0 は Windows と macOS arm64 ビルドを提供しています。Linux はソースまたは既存のビルド手順での検証が必要です。
