  ...
  <div class="caption-toolbar">
    ...
    <div class="caption-actions">
        ...
        <template #content>
          <div class="base-time">
            <div class="base-time-container">
              <a-input type="number" min="0" v-model:value="baseHH"></a-input>
          ...
          <div class="base-time">
            <div class="base-time-container">
              <a-input type="number" min="0" max="59" v-model:value="baseMM"></a-input>
          ...
          <div class="base-time">
            <div class="base-time-container">
              <a-input type="number" min="0" max="59" v-model:value="baseSS"></a-input>
          ...
          <div class="base-time">
            <div class="base-time-container">
              <a-input type="number" min="0" max="999" v-model:value="baseMS"></a-input>
              <span class="base-time-label">{{ $t('log.ms') }}</span>
            </div>
          </div>
        </template>
        <a-button type="primary" @click="changeBaseTime" :disabled="captionData.length === 0">{{
          $t('log.changeTime')
        }}</a-button>
      </a-popover>
      <a-popover :title="$t('log.exportOptions')">
        <template #content>
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.exportFormat') }}</span>
          ...
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.exportContent') }}</span>
        ...
        <a-button @click="exportCaptions" :disabled="captionData.length === 0">{{
          $t('log.export')
        }}</a-button>
      </a-popover>
      <a-popover :title="$t('log.copyOptions')">
        <template #content>
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.addIndex') }}</span>
            <a-switch v-model:checked="showIndex" />
          </div>
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.copyTime') }}</span>
            <a-switch v-model:checked="copyTime" />
          </div>
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.copyContent') }}</span>
            <a-radio-group v-model:value="contentOption">
              <a-radio-button value="both">{{ $t('log.both') }}</a-radio-button>
              <a-radio-button value="source">{{ $t('log.source') }}</a-radio-button>
              <a-radio-button value="target">{{ $t('log.translation') }}</a-radio-button>
          ...
          <div class="log-option-row">
            <span class="log-option-label">{{ $t('log.copyNum') }}</span>
        ...
        <a-button @click="copyCaptions">{{ $t('log.copy') }}</a-button>
      </a-popover>
      <a-button danger @click="clearCaptions">{{ $t('log.clear') }}</a-button>
    </div>
  </div>
  ...
  >
    <template #bodyCell="{ column, record }">
      ...
      <template v-if="column.key === 'time'">
        <div class="time-cell">
          <code class="time-start" :style="`color: ${uiColor}`">{{ record.time_s }}</code>
      ...
      <template v-if="column.key === 'content'">
        <div class="caption-content">
          ...
          <div class="caption-translation" :style="`border-left: 3px solid ${uiColor};`">
            {{ record.translation }}
          </div>
        </div>
      </template>
...
function changeBaseTime() {
  ...
  for (let i = 0; i < captionData.value.length; i++) {
    captionData.value[i].time_s = tc.getNewTimeStr(captionData.value[i].time_s, deltaMs)
    captionData.value[i].time_t = tc.getNewTimeStr(captionData.value[i].time_t, deltaMs)
  }
}
...
.log-option-row > :deep(.ant-radio-group) {
  min-width: 0;
  display: flex;
  flex-wrap: wrap;
}

.log-option-row > :deep(.ant-switch) {
  justify-self: start;
}
